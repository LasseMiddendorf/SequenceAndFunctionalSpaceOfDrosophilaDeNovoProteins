#this scirpt was used to map ECOD domains to de novo protein structures AFTER MD simulations
...
#This script generated the "denovo_ecod_alignment_afterMD.csv" file which is used in the analysis notebook
...
def parse_foldseek_results(foldseek_results_file, TmScore=0.5, evalue=0.1, tcov=0.0):
    ...
    foldseek_result_raw = pd.read_csv(foldseek_results_file, sep='\t', names=names, index_col=False, header=None)
    foldseek_result_raw["replicate"] = foldseek_result_raw["query"].apply(lambda x: x.rsplit("_", 3)[2])
    foldseek_result_raw["query"] = foldseek_result_raw["query"].apply(lambda x: x.rsplit("_", 3)[0])

    foldseek_result = pd.DataFrame(columns=foldseek_result_raw.columns)
    for id in foldseek_result_raw["query"].unique():
        selection = foldseek_result_raw[foldseek_result_raw["query"] == id]
        selection = selection.sort_values(by="alntmscore", ascending=False).reset_index(drop=True).head(1)
        foldseek_result = pd.concat([foldseek_result, selection], ignore_index=True)
